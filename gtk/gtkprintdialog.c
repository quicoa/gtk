 ...
 * function. The actual printing can be done with [method@GTk.PrintDialog.print].
 * These APIs follows the GIO async pattern, and the results can be obtained
            ...
            GTask          *task)
{
  ...
  if (self->printer)
    ...
    {
      g_task_return_new_error (task, GTK_DIALOG_ERROR, GTK_DIALOG_ERROR_FAILED, "Non printer selected");
      g_object_unref (task);
    }
}
 ...
 *
 * One possible use for this method is to have the user select
 * a printer, then show a page setup UI in the application (e.g.
 * to arrange images on a page), then call [method@Gtk.PrintDialog.print_stream]
 * on @self to do the printing without further user interaction.
 *
 * Since: 4.12
 */
void
